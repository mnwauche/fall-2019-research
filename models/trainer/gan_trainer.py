...
class GANTrainer():
    ...
    def loadSavedTraining(self,
                          ...
                          loadGOnly=False,
                          loadDOnly=False,
                          finetune=False): #setting to true will modify
                          # trainConfig file to original
        r"""
        ...
        if tmpPathLossLog is None:
            ...
            # copies the loss profile from 0 => startScale
            # startScale = tmpConfig['scale']
    ...
    def getDBLoader(self, scale):
        ...
        return torch.utils.data.DataLoader(dataset,
                                           batch_size=self.modelConfig.miniBatchSize, # default 16
    ...
    def trainOnEpoch(self,
        ...
        for item, data in enumerate(dbLoader, 0):
            ...
            # training happens here (optimizeParameters)
            import time
            start_time = time.time()
            if len(data) > 2:
                mask = data[2]
                allLosses = self.model.optimizeParameters( # see models/base_GAN.py
            ...
            # TO DO: show feature maps during training
            print(f'seconds per batch ({self.modelConfig.miniBatchSize}): {end_time - start_time}')
    ...
    def sendModelToVis(self):
        # self.model = ProgressiveGAN (contains netG and netD)
        with open('test_output.txt', 'a') as file:
            file.write(f'enumerate netG:\n')
            for idx, m in enumerate(self.model.netG.modules()):
                file.write(f'\n{idx} => {m}')

            file.write(f'\nenumerate netD:\n')
            for idx, m in enumerate(self.model.netD.modules()):
                file.write(f'\n{idx} => {m}')
